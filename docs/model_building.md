...
* **`callbacks`**: Training helpers (e.g., `recommended_callbacks()`).
* **`neglogliks`**: Negative log-likelihood functions.

## Recommended Training Pattern

```python
from pypsps.keras.models import build_toy_model, recommended_callbacks

model = build_toy_model(n_states=5, n_features=10)
callbacks = recommended_callbacks()
